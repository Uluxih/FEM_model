...
# ==========================================
# 3. Решатель с записью истории (Обновлен для перемещений)
# ==========================================
class TrackingNewtonRaphsonControl(NonLinearNewtonRaphsonControl):
    def __init__(self, model, track_nodes, num_steps=10, tol=1e-4, max_iter=20):
        super().__init__(model, num_steps, tol, max_iter)
        self.track_nodes = track_nodes  # Теперь передаем массив узлов для сбора реакций
        self.history_Ux = [0.0]
        self.history_Uz = [0.0]
        self.history_Fx = [0.0]
        self.history_Fz = [0.0]
        ...
        for step in range(1, self.num_steps + 1):
            ...
            for iteration in range(self.max_iter):
                ...
                for bc in self.model.bcs:
                    ...
                    # ИСПРАВЛЕНО: Правильный расчет инкремента перемещения для шага
                    val = (bc.value / self.num_steps) if iteration == 0 else 0.0
                    K_t[dof, :] = 0.0
                    K_t[:, dof] = 0.0
                    K_t[dof, dof] = 1.0
                    Residual[dof] = val
                ...
                if error < self.tol and iteration > 0:
                    ...
                    # Суммируем внутренние силы в заданных узлах (это и есть реакции R = F_int)
                    rx_force = sum(F_int[n.dofs[0]] for n in self.track_nodes)
                    rz_force = sum(F_int[n.dofs[2]] for n in self.track_nodes)

                    self.history_Ux.append(U_global[self.track_nodes[0].dofs[0]])
...
def run_wall_compression_test():
    ...
    # ---------------------------------------------
    # ПРИЛОЖЕНИЕ КИНЕМАТИЧЕСКИХ ВОЗДЕЙСТВИЙ
    # ---------------------------------------------
    # Задаем конечные целевые перемещения верхних узлов
    target_disp_z = -0.00001  # Заданная осадка (сжатие) по оси Z (-50 мм)
    target_disp_x = 0.0001  # Заданный сдвиг по оси X (20 мм)

    for node in top_nodes:
        model.add_bc(node, 2, target_disp_z)
        model.add_bc(node, 0, target_disp_x)

    # Запускаем решатель
    # Передаем массив top_nodes для суммирования реакций (внутренних сил)
    control = TrackingNewtonRaphsonControl(
        model=model, track_nodes=top_nodes, num_steps=5, tol=1e-4
    )
    control.solve()
    ...
    ax1.set_ylabel("Суммарная сила реакции $F_z$ (МН)")
    ...
    ax2.set_ylabel("Суммарная сила реакции $F_x$ (МН)")
