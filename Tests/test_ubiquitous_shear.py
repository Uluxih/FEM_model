...
from FEM.Integration_Point_Level.FixedHorizontalJointModel3D import FixedHorizontalJointModel3D
...
def run_multi_element_test():
    ...
    ROCK_RP = 15.5  # Предел на растяжение (Па)
    ROCK_RC = 15.0  # Предел на сжатие (Па)
    ...
    joint_params = {
        ...
        # Трение и дилатансия по образовавшейся трещине
        # ВНИМАНИЕ: 'c' и 't' удалены, так как они вычисляются из cp_material!
        'phi': 10.0,  # Угол внутреннего трения трещины (град)
        'psi': 5.0  # Угол дилатансии трещины (град)
    }
    ...
    # ---------------------------------------------------------
    # Сдвиг 0.50 м (50% деформации)
    DISPLACEMENT_X = 00.040
    ...
    # ---------------------------------------------------------
    NUM_STEPS = 60  # Количество шагов нагружения
    MAX_ITER = 250  # Макс. итераций Ньютона-Рафсона на шаг
    ...
    if len(fx_mn) > 0:
        plt.axhline(max(fx_mn), color='black', linestyle=':',
                    label=f'Пиковая несущая способность ({max(fx_mn):.6f} МН)')
